...
class BuildGraphReportTab( pagePlaces: PagePlaces,
                           server: SBuildServer,
                           descriptor: PluginDescriptor ) : ViewLogTab(TAB_TITLE, TAB_CODE, pagePlaces, server)
{
    init {
        includeUrl = descriptor.getPluginResourcesPath("results-tab.jsp")
        register()
    }
    ...
    override fun isAvailable(request: HttpServletRequest, build: SBuild): Boolean {
        ...
        val reportArtifact = buildArtifacts.findArtifact(BuildGraphConstants.ARTIFACTS_BASE_DIR + File.separator + BuildGraphConstants.ARTIFACTS_REPORT_JSON)
        return reportArtifact.isAvailable
    }

    companion object {
        private const val TAB_TITLE = "Build Graph Report"
        private const val TAB_CODE = "uebgReportTab"
    }
}
